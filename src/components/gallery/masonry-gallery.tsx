          ...
          <DialogContent className="border-none bg-transparent p-0 shadow-none">
            ...
            <div className="mt-4 flex items-center justify-between">
              <button
                data-cursor-hover
                onClick={() =>
                  setActiveIndex((i) => (i !== null ? (i - 1 + filtered.length) % filtered.length : 0))
                }
              ...
              <p className="text-sm text-muted">{filtered[activeIndex].alt}</p>
              <button
                data-cursor-hover
                onClick={() => setActiveIndex((i) => (i !== null ? (i + 1) % filtered.length : 0))}
                className="glass flex h-11 w-11 items-center justify-center rounded-full text-white transition-colors hover:text-primary"
              >
